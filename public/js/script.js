...
document.onkeydown = e => {
    if (["F12", "u", "U", "c", "v"].includes(e.key) && (e.ctrlKey || e.metaKey)) {
        alert("Don't try to inspect or copy. 😍");
        return false;
    }
};
...
// // async function displayAlbums() {
// //     const basePath = window.location.origin + "/songs/";
...
// //                     // let metadataResponse = await fetch(`/songs/${folder}/info.json`);
// //                     let metadataResponse = await fetch(`${basePath}${folder}/info.json`);
...
// //                         console.error(`Metadata missing title/description in folder: ${folder}`);
// //                         continue;
// //                     }
